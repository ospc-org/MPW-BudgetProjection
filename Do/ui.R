# User interface for MPW dashboard
# Load Packages
library(shiny)
library(ggplot2)
library(ggiraph)
...
ui <- fluidPage(
  ...
  titlePanel(title =  span(
    strong('Dashboard Companion for Warshawsky, Mantus, and Pang (2025):'), 
    em('A New Long-Run Economic Model Basis for Projecting the Finances of the US Government')),
    windowTitle = "WMP Model"),
  
  tags$div(
    em("(Read the full ",
       tags$a(href="https://www.aei.org/research-products/working-paper/a-new-long-run-economic-model-basis-for-projecting-the-finances-of-the-us-government/", 
  ...
  tags$div("This dashboard allows you to change initial conditions and key parameters of the model
    described and simulated in Warshawsky, Mantus, and Pang (2025). This serves as a tool
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      sliderInput("rgov_0_input", "Initial Real Interest Rate (%):",
                  ticks = FALSE,
                  min = 1.8, max = 2.8, value = 2.3, step = 0.1),
