...
# set directories
main_dir = "C:/AEI/Warshawsky/Dashboard/"
...
source(paste0(do_folder, "init_fct.R"))
source(paste0(do_folder, "simulation_fct.R"))
source(paste0(do_folder, "init_df_hc_el.R"))
...
# For multiple lines to the figures
# dataframe for debt projections
df_debt <- data.frame(
  year = years,
...
# dataframe for deficit projections
...
# dataframe for welfare projections
...
# Compute debt to GDP
simulate_model <- function(a1 = 0.213, 
                           g1_input = 0.018,
                           g3_input = 0.004,
                           rgov_0_input = 0.023,
                           hc_inc_el_input = 1.2,
...
server <- function(input, output, session) {
  ...
  output$debt_gdp <- renderGirafe({
    ...
    if (values$alt_counter == 0){
      ...
      gg_plot_debt <- ggplot() +
        ...
        xlim(2020, 2100) +
        labs(title = "Panel A. Projected Debt to GDP, 2023-2096 (%)",
             x = "Year",
             y = "Percentage of GDP")
      # Deficit to GDP
      gg_plot_deficit <- ggplot() +
        ...
        xlim(2020, 2100) +
        labs(title = "Panel B. Projected Deficit to GDP, 2023-2096 (%)",
             x = "Year",
             y = "Percentage of GDP")
      # NHE to GDP
      gg_plot_nhe <- ggplot() +
        ...
        xlim(2020, 2100) +
        labs(title = "Panel C. Projected NHE to GDP, 2023-2096 (%)",
             x = "Year",
             y = "Percentage of GDP")
      ...
      gg_plot_welfare <- ggplot() +
        ...
        xlim(2025, 2100) +
        labs(title = "Panel D. Projected Welfare Growth Rate, 2025-2096 (%)",
             x = "Year",
  ...
  observeEvent(input$reset_sliders, {
    ...
    updateSliderInput(session,'rgov_0_input',value = 2.3)
