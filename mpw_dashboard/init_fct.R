...
init_model <- function(g1_input = 0.018,
  ...
  df_hc_spending_mat <- read_excel("data/hc_spending_estimates.xlsx") %>%
  ...
  # import LFPR matrix
  df_lfpr <- read_excel("data/lf_proj_cbo.xlsx") %>%
  ...
  df_cbo_demo <- read_excel("data/CBO_2023_Projections_cleaned.xlsx") %>%
    mutate(gender = str_to_lower(gender))
  
  # add in rows for 2021 equal to 2022
  df_cbo_2021 <- df_cbo_demo %>%
    filter(year == min(years) + 1) %>%
    mutate (year = min(years))
  
  df_cbo_demo <- df_cbo_demo %>%
    add_row(df_cbo_2021)
  ...
  hrs_0 <- 34.0679284 # initial hours worked per week
  ...
  non_def_coeff <- 0.7051/(df_pop %>% filter(year == 2021) %>% select(population) %>% pull())
  ...
  df_hc <- df_hc_cbo_demo %>%
    ...
    mutate(phc_to_nhe = case_when( # Convert PHC estimates to NHE estimates using 2021 values
      source == 'medicare' ~ 1.072508632,
      source == 'medicaid' ~ 1.138337469,
      source == 'private_insurance' ~ 1.110331568,
      source == 'oop' ~ 1,
      source == 'other' ~ 1.067217631
    )) %>%
  ...
  r_0 <- (df_projections %>% filter(year == 2021) %>% select(r) %>% pull())
